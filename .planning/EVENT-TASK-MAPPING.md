# Pi Events → Worker/Monitor Task Mapping
...
**Key insight**:
- User triggers Worker (user input)
- Monitor DIRECTS Worker through system prompt
- User can also trigger at any point (input event)
- Worker is ALWAYS working, Monitor is ALWAYS directing

### Session Events

| Event | When | Worker Task | Monitor Task |
|-------|------|-------------|-------------|
...
### Phase 1: Essential (Worker + Monitor basic)
...
- [ ] `session_shutdown` → Worker: save state, Monitor: final report
...
| Event | Worker | Monitor | System Prompt Injection |
|-------|--------|---------|----------------------|
...
3. **Injects** work into Worker's system prompt
4. **Worker continues** based on Monitor's direction
5. **Cycle repeats**
...
Every Pi event is an opportunity for Worker or Monitor to act. The system is inherently event-driven - we just need to hook into each event and define the action.

**Goal**: Map every event → Worker or Monitor task → DB action
...
- Worker acts on user-prompted events
