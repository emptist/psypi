...
## Objective
Fix the critical daily work tools: inter-review and commit. These are the ONLY places where A-worker should be activated.
...
- Too many A-worker triggers have been removed — need to add back ONLY for inter-review and commit
...
### Task 4: Verify A-worker activation for commit
**Type**: verify
**Action**: When psypi-commit is called, A-worker should review the code
...
- Do NOT add A-worker triggers to every hook
...
## Success Criteria
- [ ] `psypi-commit` works with A-worker review
- [ ] `psypi-issue-add` works
- [ ] `psypi-areflect` works
- [ ] A-worker only activates for inter-review and commit
