# Plan: Implement Simple A-Agentbot → S-Agentbot Coordination via Direct Messaging

## Goal
Implement the core psypi coordination mechanism where the Autonomic Agentbot (A-agentbot) wakes the Somatic Agentbot (S-agentbot) by emitting a system/user prompt when `ctx.isIdle()` becomes true.
...
1. **Detect Idle State**
   - In the *Monitor* (A-agentbot), poll or listen for `ctx.isIdle()` becoming true.[User oomment: and still true after x(=3) seconds].
...
2. **Emit a Wake‑up Prompt**
   - When idle is detected, send a prompt to the S‑agentbot.
   - Use either:
     - `pi.sendMessage(message, targetAgentId)` for direct messaging to a specific S‑agentbot.
     - `pi.broadcast(message)` for a global broadcast (both work).

3. **Message Content**
   - Include a recognizable `[Monitor]` prefix so the S‑agentbot can filter it.
...
6. **Testing & Validation**
   ...
   - Verify the prompt reaches the intended S‑agentbot.
   - Confirm system prompts are injected correctly by the before_agent_start hook.

## File Structure for Implementation
- `src/monitor/agentbot_coordination.gleam` – Core idle detection & messaging logic.
- `src/monitor/agentbot_coordination.hook.gleam` – Hook registration (e.g., `tool_result` listener).
...
## Acceptance Criteria
- When an S‑agentbot finishes its turn and becomes idle, the A‑agentbot automatically sends a system prompt.
- The S‑agentbot receives the prompt and resumes work with an updated system context.
- Logs show a "[Monitor]" message appearing before the S‑agentbot’s next action.
- No breaking changes to existing functionality.

## Timeline
- **Day 1**: Draft `agentbot_coordination.gleam` and hook registration.
...
1. **`src/agentbot_coordination.gleam`** — Core Gleam module with two functions:
...
2. **`src/generator/agent_end_coordination.gleam`** — Generator module that produces JS text for the `agent_end` hook:
   ...
   - If still idle, dynamically imports `agentbot_coordination.mjs` and calls `build_wake_message()`
...
- **Used `agent_end` hook** (not `tool_result`) — `agent_end` fires once per turn when the agentbot finishes, which is the natural point to check idle state. `tool_result` fires after every tool call (mid-turn), making it unsuitable for idle detection.
...
After reading the Pi SDK extensions reference, here is what `ctx` and `pi` provide to the A-agentbot inside event hooks:

### What `ctx` provides (ExtensionContext)

| Property                          | Description                                            | Useful for A-agentbot?                                                   |
| --------------------------------- | ------------------------------------------------------ | ------------------------------------------------------------------------ |
| `ctx.isIdle()`                    | Whether agent is idle                                  | ✅ Already used for idle detection                                        |
| `ctx.getContextUsage()`           | Token usage vs model context window                    | ✅ Could decide whether to wake S-agentbot based on context fullness      |
| `ctx.sessionManager`              | Read-only access to session entries, messages, history | ✅ Could read recent conversation to understand what S-agentbot was doing |
| `ctx.model` / `ctx.modelRegistry` | Current model info, API keys                           | ℹ️ Available but not needed for coordination                              |
| `ctx.getSystemPrompt()`           | Current system prompt string                           | ℹ️ Could inspect S-agentbot's instructions                                |
...
| Method                        | Description                             | Useful for A-agentbot?                 |
| ----------------------------- | --------------------------------------- | -------------------------------------- |
...
| `pi.getCommands()`            | List available slash commands           | ℹ️ Could check what S-agentbot can do   |
...
**`ctx` and `pi` together provide rich context about the session state** — the A-agentbot can:

1. **Read conversation history** via `ctx.sessionManager.getEntries()` — understand what the S-agentbot was doing before going idle
2. **Check context usage** via `ctx.getContextUsage()` — decide if it's a good time to wake the agentbot (e.g., wait for compaction if context is full)
...
- The S-agentbot's internal state or memory

### Simplified Architecture

The A-agentbot does NOT need to query the DB or compose smart messages. The minimal pattern is:

```
agent_end → setTimeout(5s) → ctx.isIdle()
  → pi.sendMessage('[Monitor] Wake up.', { triggerTurn: true })
    → S-agentbot wakes, checks psypi-issues / psypi-tasks itself
```

The S-agentbot is smart enough to check for pending work. The A-agentbot's only job is to **wake it up**.
...
1. **Renamed `customType`** from `monitor-wake-up` to `monitor-calling` — better reflects the A-agentbot actively calling the S-agentbot.

2. **LLM now receives context** — the system prompt now includes:
   - Identity: "You are the Autonomic Agentbot (A-agentbot), the Monitor"
...
3. **Removed `agentbot_coordination.gleam` dependency** — no longer imports or calls the Gleam DB query module. The A-agentbot composes messages entirely via `callMonitor` in JS.
    ...
    → callMonitor(systemPrompt + tokenInfo)
      → LLM composes contextual [Monitor] message
        → pi.sendMessage({ customType: 'monitor-calling' }, { triggerTurn: true })
          → S-agentbot wakes, checks for work
```
...
- The S-agentbot always double-checks by calling psypi-issues/psypi-tasks itself, so hallucinations are harmless

---

*Prepared by the Autonomic Agentbot planning module. Implementation updated with context-aware message composition.*
