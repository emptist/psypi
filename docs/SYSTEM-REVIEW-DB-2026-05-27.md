...
|----------|-------|----------|
| missing_cast | 11 | 1C/4H |
| design_flaw | 8 |  |
| wrong_status | 5 | 0C/2H |
| missing_params | 5 | 0C/1H |
| ffi_mismatch | 5 | 1C/2H |
| missing_project_id | 5 | 1C/2H |
| logic_error | 4 | 0C/1H |
| error_handling | 4 |  |
| style | 3 |  |
| hardcoded_config | 3 |  |
...
| 111 | wrong_status | monitor_ai | check_system_health uses FAILED for tasks but DB has PENDING/COMPLETED | Health metrics for tasks always return 0 |
| 112 | wrong_status | monitor_ai | analyze_and_act same status value bugs as check_system_health | Analysis always misses task data |
### MEDIUM
...
**Description**: COUNT(*) returns bigint (needs ::INT), status compared as text>=int, status column missing from GROUP BY

**Evidence**: `broadcast.gleam stats(): multiple query errors`
...
**Description**: content is text (ok) but reference_list is jsonb, needs ::text for Gleam decoder

**Evidence**: `skill.gleam get(): reference_list decoded without ::text cast`
...
**Description**: exec_sync(command) runs shell command directly; no input sanitization

**Evidence**: `pi_extension_ffi.mjs: exec_sync(cmd)`
...
**Description**: tasks.status CHECK allows PENDING/COMPLETED only; FAILED does not exist

**Evidence**: `monitor_ai.gleam check_system_health(): WHERE status != 'FAILED'`

**Impact**: Health metrics for tasks always return 0
...
**Description**: Same FAILED status issue propagated to analyze_and_act

**Evidence**: `monitor_ai.gleam analyze_and_act(): same wrong status`

**Impact**: Analysis always misses task data
