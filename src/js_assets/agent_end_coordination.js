    // A-worker coordination: detect idle → check context → compose → wake up S-worker
      ...
      setTimeout(async () => {
        try {
          ...
          if (ctx.hasPendingMessages()) {
            ctx.ui.notify('[AUTONOMIC] S-worker has pending messages, skipping wake-up', 'info');
            return;
          }
          ...
          try {
            ...
            const messages = [{ role: 'user', content: [{ type: 'text', text: 'Somatic worker is idle. Compose a wake-up message based on the recent context.' }], timestamp: Date.now() }];
