...
pub fn handler_body() -> String {
  [
    ...
    "    try {\n",
    "      // Read debounce from DB — always from DB, no hardcoded fallback\n",
    "      let debounceMs;\n",
    "      {\n",
    ...
    "        }\n",
    "        if (!debounceMs) {\n",
    "          throw new Error('monitor_debounce_ms not found or invalid in system_config');\n",
    "        }\n",
    "      }\n",
    ...
    "              let brief = '';\n",
    "              try { brief = fs.readFileSync(briefPath, 'utf-8'); } catch(e) { ctx.ui.notify('[AUTONOMIC] No MONITOR‑BRIEF.md found', 'info'); }\n",
