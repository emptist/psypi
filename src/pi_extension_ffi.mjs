...
export function registerAutonomicWakeupRenderer(pi) {
  pi.registerMessageRenderer('autonomic-wakeup', (message, { expanded }, theme) => {
  ...
  pi.registerMessageRenderer('autonomic-error', (message, { expanded }, theme) => {
    let text = theme.fg('error', '[A-agentbot ERROR] ');
    text += theme.fg('error', message.content);
    if (expanded && message.details) {
      text += '\n' + theme.fg('dim', JSON.stringify(message.details, null, 2));
    }
    const box = new Box(1, 1, (t) => theme.bg('customMessageBg', t));
