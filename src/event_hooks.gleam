...
fn event_hook_decoder() -> decode.Decoder(EventHook) {
  ...
  use worker_action <- decode.field("worker_action", decode.optional(decode.string))
  use injection_enabled <- decode.field("injection_enabled", decode.bool)
  use description <- decode.field("description", decode.optional(decode.string))
  use last_triggered <- decode.field("last_triggered", decode.optional(decode.string))
  ...
  decode.success(EventHook(
    ...
    hook_status:,
    monitor_action:,
    worker_action: opt_to_str(worker_action),
    injection_enabled:,
  ...
  db.with_connection(fn(conn) {
    let sql = "
      SELECT id::text, event_name, hook_status, monitor_action,
             COALESCE(worker_action, '') as worker_action,
             injection_enabled,
...
pub fn list_active_hooks() -> promise.Promise(Result(List(EventHook), db.DbError)) {
  db.with_connection(fn(conn) {
    let sql = "
      SELECT id::text, event_name, hook_status, monitor_action,
             COALESCE(worker_action, '') as worker_action,
...
pub fn record_trigger(event_name: String) -> promise.Promise(Result(Nil, db.DbError)) {
  db.with_connection(fn(conn) {
    let sql = "
...
pub fn record_error(event_name: String, error_msg: String) -> promise.Promise(Result(Nil, db.DbError)) {
  db.with_connection(fn(conn) {
    let sql = "
      UPDATE psypi_event_hooks
...
pub fn set_hook_status(event_name: String, status: String) -> promise.Promise(Result(Nil, db.DbError)) {
  db.with_connection(fn(conn) {
    let sql = "
      UPDATE psypi_event_hooks
...
pub fn format_hooks_summary(hooks: List(EventHook)) -> String {
  ...
  |> list.map(fn(h) {
    ...
    status_icon <> " " <> h.event_name <> inj <> "\n    Monitor: " <> h.monitor_action
  })
  |> string.join("\n\n")
}
