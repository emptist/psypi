...
pub type MonitorModel {
  MonitorModel(provider: String, model: Option(String))
}
...
pub fn get_model() -> promise.Promise(
  Result(Option(MonitorModel), MonitorError),
) {
  db.with_connection(
    fn(conn) {
      let sql =
            ...
            case result.rows {
              ...
              [row, ..] -> {
                case decode.run(row, monitor_model_decoder()) {
                  Ok(m) -> Ok(Some(m))
                  Error(_) ->
                    Error(DecodeError("Failed to decode monitor model"))
                }
              }
            }
          }
      ...
      promise.await(db.query(conn, reset_sql, reset_params), fn(reset_result) {
        case reset_result {
          ...
          Ok(_) -> {
            ...
            promise.await(
              db.query(conn, update_sql, update_params),
              fn(update_result) {
                case update_result {
                  Error(e) ->
                    promise.resolve(Error(db_error_to_monitor_error(e)))
...
// ---------------------------------------------------------------------------
// Notifications for Agentbot communication
// ---------------------------------------------------------------------------
...
/// Create a notification for an agent (Monitor → Agentbot communication)
            ...
            case result.rows {
              [row, ..] -> {
                case decode.run(row, id_only_decoder()) {
                  Ok(id) -> Ok(id)
                  Error(_) ->
                    Error(DecodeError("Failed to decode notification id"))
                }
              }
